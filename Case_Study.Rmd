...
## Loading data
Here I'm using a dataset that was corrected using Google's Geocoder as the original latitude/longitude was not accurate and some of the houses were mapped to being in the ocean.
...
Even though there are 39 missing values when we use IQR method I think looking at those specific outliers I think it is not a product of bad/corrupt data. It is just that these houses are further from city centers and in more distant neighborhoods which is why their prices are drastically different.
...
Here I combine features like rooms per dwelling and distance from Boston employment center to have a measure like how much residential space is available per distance from the city center.
...
## Clustering tools
Below I am going to apply two different types of clustering methods: k-means and k-medoids.
